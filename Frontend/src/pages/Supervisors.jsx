...
const Supervisors = () => {
    ...
    const [newSupervisorPersonID, setNewSupervisorPersonID] = useState(null);
    const refs = useRef([]);
    ...
    const handleAddNewSubmit = (e) => {
        ...
        axios.post(`${serverAddress}/supervisor`, {
            name: newSupervisorName,
            personId: newSupervisorPersonID
        })
            .then((response) => {
    ...
    const handleUpdateButton = (id, refIndex) => {
        ...
        axios.put(`${serverAddress}/supervisor/${id}`, {
    ...
    const handleRemoveButton = async (id) => {
        ...
        axios.delete(`${serverAddress}/supervisor/${id}`)
            .then((response) => {
                window.location.reload();
            })
            .catch(error => console.error(error));
    ...
    return (
        <div className="p-3">
            ...
            <div className="grid gap-6 mb-6 md:grid-cols-1">
                {
                    supervisors && supervisors.map((data, index) =>
                        <div className="grid gap-6 mb-1 md:grid-cols-1">
                            <div className="relative">
                                <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                                    {preferences.theme === "dark" ? <MdOutlineSupervisorAccount size="1.5em"/> : <MdSupervisorAccount size="1.5em"/>}
                                </div>
                                <div className="absolute inset-y-11 start-0 flex items-center ps-3 pointer-events-none text-sm">
                                    {preferences.theme === "dark" ? <IoPersonCircleOutline size="0.9em"/> : <IoPersonCircle size="0.9em"/>}
                                    {persons && persons.find(i => i.PersonID === data.PersonID).Name}
                                </div>
                                <input type="text" id={data.SupervisorID}
                                ...
                                <button type="button" className="text-white absolute end-24 mr-1 bottom-2.5 bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                                        onClick={() => handleUpdateButton(data.SupervisorID, index)}>
                                    Update
                                </button>
                                <button type="button" className="text-white absolute end-2.5 bottom-2.5 bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
                                        onClick={() => handleRemoveButton(data.SupervisorID)}>
                                    Remove
                                </button>
                            </div>
                        </div>
                ...
                <div className="grid gap-6 mb-6 md:grid-cols-1 border p-3 rounded bg-gray-100 dark:bg-gray-900">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Type new supervisor name here..." required/>
                    </div>
                    <div>
                        ...
                        <select id="newSupervisorPerson"
                                value={newSupervisorPersonID}
                                onChange={e => setNewSupervisorPersonID(e.target.value)}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                required>
                            <option value="">Choose a persons...</option>
                            {
                                persons && persons.map(i => <option key={i.PersonID} value={i.PersonID}>{i.Name}</option>)
                            }
                        </select>
                    </div>
                </div>
